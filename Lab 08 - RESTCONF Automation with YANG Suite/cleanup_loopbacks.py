...
for item in data["loopbacks"]:
    url = base + item_uri(data, item["name"])
    print("DELETE", url)
    if not args.apply:
        continue
    ...
    if item["description"] not in current.text:
        raise RuntimeError(
            f"Refusing to remove {item['name']}: ownership description differs"
        )
    response = session.delete(url, timeout=30)
    require(response, {200, 204})
    print(" ", response.status_code)
if not args.apply:
    print("Dry run only. Use --apply to remove lab-owned interfaces.")
