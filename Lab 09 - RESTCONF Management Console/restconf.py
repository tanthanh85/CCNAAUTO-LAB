...
class IOSXE:
    ...
    def request(self, method, uri, allow_not_found=False, **kwargs):
        try:
            response = self.session.request(method, self.base + uri, timeout=12, **kwargs)
    ...
    def routes(self):
        ...
        for item in items:
            ...
            if isinstance(next_hops, dict): next_hops = [next_hops]
            if not next_hops: next_hops = [{"fwd": ""}]
            for hop in next_hops:
                result.append({"prefix": item.get("prefix", ""), "mask": item.get("mask", ""),
    ...
    def put_route(self, route):
        ...
        uri = config.ROUTE_ITEM_URI.format(
            prefix=quote(prefix, safe=""), mask=quote(mask, safe=""),
    ...
    def loopbacks(self):
        ...
        for item in items:
            ...
            result.append({"number": int(item.get("name", item.get("number", 0))),
                           "description": item.get("description", ""),
                           "address": primary.get("address", ""), "mask": primary.get("mask", "")})
        return result

    def put_loopback(self, item):
        number = validate_loopback(item)
        payload = {"Cisco-IOS-XE-native:Loopback": {"name": number,
            "description": item.get("description", "LAB9_MANAGED"),
            "ip": {"address": {"primary": {"address": item["address"], "mask": item["mask"]}}}}}
        self.request("PUT", config.LOOPBACK_ITEM_URI.format(number=number), json=payload)

    def delete_loopback(self, item):
        number = int(item["number"])
        if number == 0: raise ValueError("Loopback0 is protected")
    ...
    def counters(self):
        ...
        return {"time": time.time(), "in_octets": integer(stats, "in-octets-64", "in-octets"),
...
def validate_route(item):
    ...
    if network.version != 4: raise ValueError("Only IPv4 static routes are supported")
    if network.prefixlen == 0: raise ValueError("The default route is protected")
    if ipaddress.ip_address(item["next_hop"]).version != 4: raise ValueError("Next hop must be IPv4")
    return network


def validate_loopback(item):
    number = int(item["number"])
    if not 1 <= number <= 2147483647: raise ValueError("Loopback number must be greater than 0")
    address = ipaddress.ip_address(item["address"])
    mask = ipaddress.ip_address(item["mask"])
    if address.version != 4 or mask.version != 4: raise ValueError("Loopback address and mask must be IPv4")
    return number
